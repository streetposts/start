...
def printQuery(query):
  output = ""
  for row in query: 
    shortTime = str(row[2][:5])
    if int(shortTime[:2]) > 12 :
      hours = int(shortTime[:2])
      hours -= 12
    output += str( str(hours) + ":" + row[2][3:5] +" " + str(row[0])+ " " +  str(row[1]) + " " +'\n')
  return(output)
